...
class SGFN(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop: str = 0.,
                 use_lora=True, r=8):
        super().__init__()
        ...
        #     self.fc2 = loralib.Linear(hidden_features // 2, out_features, r=r)
        # else:
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads,
                 ...
                 drop=0., attn_drop=0., rg_idx=0, b_idx=0, use_lora=True, r=8):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.split_size = split_size
...
class Adaptive_Channel_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., use_lora=False,
                 r=8):
        super().__init__()
        self.num_heads = num_heads
...
class DATB(nn.Module):
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
                 qkv_bias=False, qk_scale=None, drop=0., use_lora=True, r=8,
        ...
        if b_idx % 2 == 0:
            ...
            self.attn = Adaptive_Spatial_Attention(
                ...
                qk_scale=qk_scale,
                drop=drop, attn_drop=attn_drop, rg_idx=rg_idx, b_idx=b_idx, use_lora=use_lora, r=r
            )
        else:
            # DCTB
            self.attn = Adaptive_Channel_Attention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                proj_drop=drop, use_lora=use_lora, r=r,
            )
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            DATB(
                ...
                b_idx=i,
                r=r,
                use_lora=True if (i in use_lora_block and use_lora) else False
...
class DATv2_3_0_3(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
            layer = ResidualGroup(
                ...
                use_lora=True if i in lora_layers else False,
                r=rank,
            )
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x, x_size)
            features.append(rearrange(self.norm(x), 'b (h w)c -> b c h w', h=H, w=W))
        x = self.norm(x)
        x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W)
